...
class UserController extends AdminController
{
     ...
     *访问预约
    */
    public function vistBooking(){
        $vistbooking = M('vistbooking'); // 实例化User对象
        $count = $vistbooking->count();
        $Page       = new \Think\Page($count,15);
        $show       = $Page->show();
        $data = $vistbooking->order('create_time DESC')->limit($Page->firstRow.','.$Page->listRows)->select();
     ...
     *访问预约删除
    */
    public function delVistBooking($id){
        $vistbooking = M('vistbooking');
        $res=$vistbooking->delete($id);
        if ($res) {
            $this->success('删除成功！','',1);
        } else {
            $this->error('删除失败！');
